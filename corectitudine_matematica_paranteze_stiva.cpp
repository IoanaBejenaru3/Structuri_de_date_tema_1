...
#include <iostream>
//Sa se verifice daca un set de paranteze citit de la tastatura este corect din punct de vedere matematic.
//corect: {[()()][()]}
//gresit: {(})[{}]
struct Nod{
    Nod* next;
...
int main()
{
    ...
    char c;
    bool paranteze_rotunde=0, paranteze_patrate=0, acolade=0;
    //aceste variabile bool sunt pentru a ne asigura ca ordinea parantezelor este corecta
    //in sens matematic: { [ () ] () () [] }
    std::cin >> n;
    for(int i=0; i<n; i++)
    {
        ...
        if(c=='{')
        {
            if(acolade==1 || paranteze_patrate==1 || paranteze_rotunde==1) {std::cout << "Parantezele NU sunt corecte din punct de vedere matematic!"; return 0;}
            acolade=1; //avem deschisa o acolada
            Adauga('{');
        }
        else if(c=='[')
        {
            if(paranteze_patrate==1 || paranteze_rotunde==1) {std::cout << "Parantezele NU sunt corecte din punct de vedere matematic!"; return 0;}
            paranteze_patrate=1;
            Adauga('[');
        }
        else if(c=='(')
        {
            if(paranteze_rotunde==1) {std::cout << "Parantezele NU sunt corecte din punct de vedere matematic!"; return 0;}
            paranteze_rotunde=1;
            Adauga('(');
        }
        else if(c==')')
        {
            if(L->info!='(') {std::cout << "Parantezele NU sunt corecte din punct de vedere matematic!"; return 0;}
            paranteze_rotunde=0;
            Stergere(1);
        }
        else if(c==']')
    ...
    std::cout << "Parantezele SUNT CORECTE din punct de vedere matematic!";
    return 0;
}
